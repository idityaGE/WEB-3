...
const privateKey = new Uint8Array([63, 201, 192, 248, 41, 146, 106, 67, 32, 168, 217, 215, 181, 136, 87, 154, 57, 128, 148, 221, 76, 49, 102, 45, 102, 16, 241, 85, 82, 152, 138, 246, 90, 237, 249, 124, 129, 215, 55, 193, 199, 27, 166, 82, 94, 45, 116, 239, 154, 185, 22, 235, 145, 2, 235, 214, 237, 157, 194, 36, 59, 89, 206, 37])
...
const seedPhrase = "rail unique humble truck divert tide hub jungle vendor advance crucial fun"
...
//     // Check if keys match
//     const match = validateKeypair.publicKey.toBase58() === keypair.publicKey.toBase58();
...
console.log("Keys match:", solanaDerivedKeypair.publicKey.toBase58() === keypair.publicKey.toBase58());
// Print both secret keys to compare
console.log("\nOriginal secret key:", Buffer.from(privateKey).toString('hex'));
console.log("Derived secret key:", Buffer.from(solanaDerivedKeypair.secretKey).toString('hex'));